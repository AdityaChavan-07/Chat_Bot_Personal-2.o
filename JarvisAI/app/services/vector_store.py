...
class VectorStoreService:
    ...
    def load_learning_data(self) -> List[Document]:
        ...
        for file_path in Path(LEARNING_DATA_DIR).glob("*.txt"):
            try:
                ...
                logger.warning("could not load learning data from %s: %s", file_path, e)
    ...
    def create_vector_store(self) -> FAISS:
        ...
        if not all_documents:
            ...
            logger.info("[VECTOR] FAISS index built successfully with %d chunks", len(chunks))
    ...
    def get_retriever(self, k: int=10):
        ...
        if k not in self._retrieve_cache:
            self._retrieve_cache[k] = self.vector_store.as_retriever(search_kwargs={"k": k})
       
        return self._retrieve_cache[k] 