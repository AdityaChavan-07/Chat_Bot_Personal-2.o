...
import uuid
import threading
from config import CHATS_DATA_DIR,MAX_CHAT_HISTORY_TURNS,CAMERA_CAPTURES_DIR,GROQ_API_KEYS,GENERAL_CHAT_ADDENDUM
...
def _save_camera_image(img_base64: str,session_id:str) -> Optional[Path]:
    ...
    try:
        ...
        logger.info("[VISION] Saved camera capture: %s (%d bytes) -> %s", filepath.name, len(data),filepath)
        return filepath 
...
SAVE_EVERY_N_CHUNKS = 5  # Save chat session to disk every N messages to prevent data loss
...
class ChatService:
    ...
    def __init__(
     ...
     """ store reference to the groq service and realtime service (if provided) for use in chat operations """
     ...
     self.sessions: Dict[str, List[ChatMessage]] = {}  # In-memory store for chat sessions, keyed by session_id
     self._save_lock = threading.Lock()
    
    def load_session_from_disk(self, session_id: str) -> bool:
        
        safe_session_id = session_id.replace("-","").replace(" ","_")  # Sanitize session_id for file naming
    ...
    def validate_session_id(self, session_id: str) -> bool:
        """ Validate that the session_id is a valid UUID string """
    ...
    def get_or_create_session(self, session_id: Optional[str] = None) -> str:
        ...
        if not  session_id:
         ...
         logger.info("[TIMING] session_get_or_crate: %.3fs (new)", time.perf_counter() - t0)
         return new_session_id
        
        if not self.validate_session_id(session_id):
            raise ValueError(f"Invalid session_id format:{session_id}. Must be a valid UUID string without special characters.")
        
        if session_id in self.sessions:
            logger.info("[TIMING] session_get_or_crate: %.3fs (memory)", time.perf_counter() - t0)
            return session_id
        
        if self.load_session_from_disk(session_id):
            logger.info("[TIMING] session_get_or_crate: %.3fs (disk)", time.perf_counter() - t0)
            return session_id
        
        self.sessions[session_id] = []
        logger.info("[TIMING] session_get_or_crate: %.3fs (new_id)", time.perf_counter() - t0)
        return session_id
    
    def add_message(self,session_id: str, role:str, content:str):
        """ Add a message to the specified session's chat history """
    ...
    def format_history_for_llm(self,session_id :str, exclude_last: bool = False) -> List[tuple]:
        """ Format the chat history for input to the LLM, optionally excluding the last message """
        
        messages = self.get_chat_history(session_id)
        history = []
        messages_to_include = messages[:-1] if exclude_last else messages
        i = 0
        while i < len(messages_to_include) - 1:
            user_msg = messages_to_include[i]
            ai_msg = messages_to_include[i + 1]
    ...
    def process_message_streaming(self, session_id: str, user_message: str) -> str:
        
        if not self.realtime_service:
            raise ValueError("Realtime service is not configured for streaming responses.")
    ...
    def process_realtime_message_stream(
        ...
        try:
            for chunk in self.realtime_service.stream_response(
                ...
                if chunk_count ==0:
                    elapsed_ms = int((time.perf_counter() - t0) * 1000)
                    yield{"_activity": {"events":"first_chunk","route":"realtime","elapsed_ms":elapsed_ms}}
    ...
    def process_jarvis_message_stream(
        ...
        if imgbase64 and CAMERA_BYPASS_TOKEN in (user_message or ""):
            ...
            yield {"_activity": {"event": "routing", "vision": True}}
        ...
        if category in (CATEGORY_TASK, CATEGORY_MIXED): 
            ...
            if self.task_executor and instant_intents:
                ...
                has_instant_actions = (
                    instant_response.wopens or
                    instant_response.plays or
                    instant_response.googlesearches or
                    instant_response.youtubesearches or
    ...
    def save_chat_session(self, session_id: str,log_timing:bool = True):
        """ Save the chat session to disk as a JSON file """
        ...
        logger.error("Failed to save chat session %s after %d attempts: %s", session_id, max_retries, last_exc)
