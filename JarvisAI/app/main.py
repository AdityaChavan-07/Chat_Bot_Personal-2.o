...
async def lifespan(app: FastAPI):
    ...
    global task_executor, vision_service, task_manager, chat_service
    print_title()
    logger.info("=" * 60)
    ...
    logger.info("[config] Tavily API Key: %s", "Loaded" if TAVILY_API_KEY else "Not Loaded")
    logger.info("[config] Image generation: pollination.ai (free,no API key)")
    logger.info("[config] Embedding Model: %s", EMBEDDING_MODEL)
    logger.info("[config] chunk size: %d | chunk overlap: %d | max chat history turns: %d", CHUNK_SIZE, CHUNK_OVERLAP, MAX_CHAT_HISTORY_TURNS)
    ...
    try:
        ...
        logger.info("[TIMING] startup - vector store initialized in %.3f seconds", time.perf_counter() - t0)
        ...
        logger.info("Initiallizing Realtime Groq Service (With Tavily search)...")
...
async def api_info():
    return {
        ...
        "endpoints":{
            "/chat":"Generaln chat(pure LLM, no web search)",
            "/chat/stream":"General chat with (streaming chunks)",
            "/chat/realtime":"Resltime chat (with Tavily search)",
            "/chat/jarvis/stream":"Jarvis unified route (two stage brain: classify -> route -> execute/stream)",
            "/chat/history/{session_id}":"Get chat history",
            "/tasks/{task_id}":"Get status of background task",
            "/health":"System health check",
            "/tts":"text to speech endpoint (for testing TTS generation)"
...
async def health():
    ...
    try:
        ...
        logger.warning("[API /health] Health check failed: %s", e)
...
async def chat(request: ChatRequest):
    ...
    logger.info("[API /chat] Incoming | session_id=%s | message_length= %s | message: %.100s", request.session_id or "new", len(request.message), request.message)
    
    try:
        session_id = chat_service.get_or_create_session(request.session_id)
        response_text = chat_service.process_message(session_id,request.message)
        chat_service.save_chat_session(session_id)
        logger.info("[API /chat] Done | session_id=%s| response_length=%s", session_id[:12], len(response_text))
        ...
        logger.warning("[API /chat] All Groq APIs failed: %s", e)
        ...
        logger.error("[API /chat] Error processing chat: %s", e, exc_info=True  )
...
_ABBREV_HOLD_RE = re.compile(r"\b(Mr|Ms|Dr|Jr|Sr|Inc|etc|i\.e|e\.g|vs|etc)\.$", re.IGNORECASE)
...
def _stream_generator(session_id: str , chunk_iter, is_reatime: bool, tts_enabled: bool = False):
    ...
    def _submit(text):
        ...
        audio_queue.append((future, text))
    ...
    def _drain_ready():
        ...
        while audio_queue and audio_queue[0][0].done():
            ...
            try:
                ...
                logger.error("[TTS-INLINE] FAILED for '%s':%s", sent[:40],exc)
        return events
    ...
    try:
        ...
        for chunk in chunk_iter:
            ...
            yield from _yield_completed_audio()
            sentences, buffer = _split_sentence(buffer)
        ...
        logger.error("[STREAM-ERROR] Exception in stream generator: %s", str(e), exc_info=True)
    ...
    if tts_enabled:
        ...
        for fut, sent in audio_queue:
            ...
            try:
                ...
                logger.warning("[TTS-INLINE] Failed for '%s': %s", (sent or "")[:40])
...
async def chat_stream(request: ChatRequest):
    ...
    try:
        ...
        return StreamingResponse(
            _stream_generator(session_id,chunk_iter, is_reatime=False,tts_enabled=request.tts),
...
async def chat_realtime(request: ChatRequest):
    if not chat_service:
        raise HTTPException(status_code=503, detail="Caht service not initialized")
    ...
    try:
        session_id = chat_service.get_or_create_session(request.session_id)
        response_text = chat_service.process_message_streaming(session_id, request.message)
        chat_service.save_chat_session(session_id)
        logger.info("[API /chat/realtime] Done | session_id=%s | response_len=%d" , session_id[:12], len(response_text))
        return ChatResponse(response=response_text, session_id=session_id)
    
    except ValueError as e:
        logger.warning("[API /chat/realtime ]Invalid session_id:{e}")
        raise HTTPException(status_code=400, detail=str(e))
        ...
        if _is_rate_limit_error(e):
            logger.warning("[API /chat/realtime]Rate limit hit:{e}")
            raise HTTPException(status_code=429, detail=RATE_LIMIT_MESSAGE)
        logger.error("[API /chat/realtime] Error processing realtime chat: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error processing chat: {str(e)}")
...
async def chat_realtime_stream(request: ChatRequest):
    ...
    try:
        ...
        return StreamingResponse(
            _stream_generator(session_id, chunk_iter, is_reatime=True, tts_enabled=request.tts),
            media_type="text/event-stream",
            headers={"Cache-Control": "no-cache", "X-Accel-Buffering":"no"},
        )
        ...
        logger.error("[API /chat/realtime/stream] Error processing realtime stream: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error processing chat: {str(e)}")
...
async def chat_jarvis_stream(request: ChatRequest):
    ...
    try:
        session_id = chat_service.get_or_create_session(request.session_id)
        chunk_iter = chat_service.process_jarvis_message_stream(session_id, request.message,imgbase64=request.imgbase64)
        return StreamingResponse(
            _stream_generator(session_id, chunk_iter, is_reatime=True, tts_enabled=request.tts),
            media_type="text/event-stream",
            headers={"Cache-Control": "no-cache", "X-Accel-Buffering":"no"},
        ...
        logger.error("[API /chat/jarvis/stream] Error processing Jarvis stream: %s", e, exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error processing chat: {str(e)}") 
...
async def text_to_speech(request: TTSRequest):
    ...
    async def generate():
        try:
            ...
            logger.error("[TTS] Error generating TTS: %s", e)
