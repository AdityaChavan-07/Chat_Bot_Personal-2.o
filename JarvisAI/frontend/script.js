...
const searchSendBtn      = $('search-send-btn');
...
async function preloadStarterAudio() {
    ...
    for (const file of PRE_STARTER_FILES) {
        try {
            const r = await fetch(`${base}/audio/${file}.mp3`);
            if (!r.ok) continue;
...
async function requestMicrophoneAccess() {
    ...
    try {
        ...
        if (isPermissionDenied) {
            ...
            showToast('Unable to access microphone. ' + msg);
        }
        return false;
    }
}
    ...
    return navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' }, audio: false })
        .then(stream => {
            ...
            if (camBtn) {
                ...
                if (iconActive) iconActive.classList.remove('is-hidden');
            }
        })
        .catch(err => {
            const msg = (err && err.message) ? err.message : '';
            ...
            if (isPermissionDenied) {
                ...
                if (camBtn) camBtn.title = 'Camera access denied. Allow in browser settings.';
            } else {
                showToast('Could not access camera. ' + msg);
            }
            throw err;
        });
}
...
function stopCamera() {
    ...
    if (camBtn) {
        ...
        if (iconActive) iconActive.classList.add('is-hidden');
    }
}
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    if (data.activity) {
                        ...
                        if (activityToggle) activityToggle.classList.remove('is-hidden');
...
async function checkHealth() {
    try {
        ...
        const status = d && typeof d.status === 'string' ? d.status.toLowerCase() : '';
...
function bindEvents() {
    ...
    if (searchSendBtn) {
        searchSendBtn.addEventListener('click', () => { if (!isStreaming) performSearch(); });
    }
    if (searchInput) {
        searchInput.addEventListener('keydown', e => {
            if (e.key === 'Enter' && !e.shiftKey) {
                e.preventDefault();
                if (!isStreaming) performSearch();
            }
        });
    }
...
function performSearch() {
    ...
    if (searchResultsWidget) {
        searchResultsWidget.classList.add('open');
    }
    if (searchResultsToggle) {
        searchResultsToggle.classList.remove('is-hidden');
    }
    updatePanelOverlay();
    sendMessage(query);
    searchInput.value = '';
...
function newChat() {
    ...
    if (searchResultsToggle) searchResultsToggle.classList.add('is-hidden');
    if (activityPanel) activityPanel.classList.remove('open');
    if (settingsPanel) settingsPanel.classList.remove('open');
    if (activityToggle) activityToggle.classList.add('is-hidden');
    if (activityList) {
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    if (data.search_results) {
                        ...
                        if (searchResultsToggle) searchResultsToggle.classList.remove('is-hidden');
